...
describe('Analytics - calculateCurrencySummary', () => {
  ...
  it('should calculate profit with MWK transactions only', async () => {
    ...
    const revenueCatId = await createTestCategory('REVENUE');
    const expenseCatId = await createTestCategory('EXPENSE');

    // Create transactions
    await createTestTransaction(propertyId, revenueCatId, {
      type: 'REVENUE',
      amount: 150000,
      currency: 'MWK',
    });
    ...
    await createTestTransaction(propertyId, expenseCatId, {
      type: 'EXPENSE',
      amount: 50000,
      currency: 'MWK',
    });
...
function calculateCurrencySummary(
  transactions: { currency: string; type: string; amount: number }[]
) {
