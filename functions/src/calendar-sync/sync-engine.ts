...
import { parseICalData, filterFutureEvents, ParsedEvent } from './ical-parser';
 ...
 */
export async function syncCalendar(propertyId: string): Promise<SyncResult> {
  ...
  try {
    ...
    const calendarSyncDoc = await db.collection('calendar_syncs').doc(propertyId).get();
    ...
    await db.collection('calendar_syncs').doc(propertyId).update({
...
async function processEvent(
  ...
  if (!existingBookingsSnapshot.empty) {
    ...
    const datesChanged =
      new Date(bookingData.checkInDate).getTime() !== event.startDate.getTime() ||
  ...
  const conflictResolved = await resolveConflicts(
    propertyId,
    event.startDate,
    event.endDate,
    event.uid,
 ...
 */
async function getOrCreateGuest(event: ParsedEvent, platform: string): Promise<string> {
