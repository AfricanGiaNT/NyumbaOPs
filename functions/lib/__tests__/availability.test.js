...
describe('Availability - ensurePropertyAvailable', () => {
    it('should allow booking when property is available', async () => {
        ...
        await expect(ensurePropertyAvailable(propertyId, tomorrow, dayAfter)).resolves.not.toThrow();
    });
    it('should prevent booking when dates overlap with existing booking', async () => {
        ...
        await (0, test_helpers_1.createTestBooking)(propertyId, guestId, {
            checkInDate: day2,
            checkOutDate: day3,
            status: 'CONFIRMED',
        });
        // Try to book overlapping dates (day1 to day3)
        await expect(ensurePropertyAvailable(propertyId, day1, day3)).rejects.toThrow('Property is not available for these dates');
        // Try to book overlapping dates (day2 to day4)
        await expect(ensurePropertyAvailable(propertyId, day2, day4)).rejects.toThrow('Property is not available for these dates');
    });
    it('should allow same-day turnover (check-out day is available for new check-in)', async () => {
        ...
        await expect(ensurePropertyAvailable(propertyId, day2, day3)).resolves.not.toThrow();
    });
    it('should ignore CANCELLED bookings when checking availability', async () => {
        ...
        await expect(ensurePropertyAvailable(propertyId, day1, day2)).resolves.not.toThrow();
    });
    it('should allow updating existing booking with excludeBookingId', async () => {
        ...
        const day3 = new Date(Date.now() + 72 * 60 * 60 * 1000).toISOString().split('T')[0];
        // Create booking
        const bookingId = await (0, test_helpers_1.createTestBooking)(propertyId, guestId, {
            checkInDate: day1,
            checkOutDate: day2,
            status: 'CONFIRMED',
        });
        // Should allow updating same booking dates (excluding itself)
        await expect(ensurePropertyAvailable(propertyId, day1, day3, bookingId)).resolves.not.toThrow();
    });
    it('should reject invalid date ranges', async () => {
        ...
        await expect(ensurePropertyAvailable(propertyId, day2, day1)).rejects.toThrow('Check-out must be after check-in');
    });
    it('should reject inactive properties', async () => {
        const propertyId = await (0, test_helpers_1.createTestProperty)({ status: 'INACTIVE' });
        const day1 = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0];
        const day2 = new Date(Date.now() + 48 * 60 * 60 * 1000).toISOString().split('T')[0];
        await expect(ensurePropertyAvailable(propertyId, day1, day2)).rejects.toThrow('Property is not active');
    });
});
